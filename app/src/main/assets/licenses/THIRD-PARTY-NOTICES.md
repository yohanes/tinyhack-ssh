...
| terminfo entries (`xterm-kitty`, `xterm-ghostty`, `ghostty`, `kitty`, `xterm-256color`) | — | Ghostty/kitty/ncurses upstream licenses | Ghostty MIT above; `licenses/ncurses-6.5.COPYING`; GPLv3 text bundled with the GPL components | compiled assets plus reproducible source in `terminfo-sources/` |
